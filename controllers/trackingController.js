...
exports.getTrackingData = async (req, res) => {
  ...
  if (!externalApiUrl || typeof externalApiUrl !== "string") {
    return res
      .status(400)
      .json({ error: "Invalid or missing external API URL" });
  }
...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    if (!user_id || !year) {
      return res
        .status(400)
        .json({ error: "Missing user_id or year parameter" });
    }
    ...
    const last7DaysQuery = knex("dba.xtrack_log")
      .select(
        knex.raw("TO_CHAR(api_date, 'Dy') as day"),
        knex.raw("COUNT(log_id) as count")
      )
      .where("user_id", user_id)
      .whereNotIn("api_request", ["login", "logout"]) // Exclude login/logout
    ...
    const trackRatioPromise = knex("dba.xtrack_log")
      .select(
        ...
        knex.raw(
          `SUM(CASE WHEN menu_id = 'Air' THEN 1 ELSE 0 END) AS "Schedule"`
        )
      )
      .where("user_id", user_id)
      .andWhereRaw("EXTRACT(YEAR FROM api_date) = ?", [targetYear])
